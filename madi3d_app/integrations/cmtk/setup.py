...
def _extract_macos_archive(archive_path, install_root, log, *, cancel_check=None):
    ...
    upstream_launcher = staging / CMTK_MACOS_UPSTREAM_LAUNCHER_RELPATH
    if not upstream_launcher.is_file():
        shutil.rmtree(staging, ignore_errors=True)
        raise CMTKUnavailableError(
            f"The CMTK archive did not contain the expected launcher at "
            f"{CMTK_MACOS_UPSTREAM_LAUNCHER_RELPATH}."
        )
    # Dispatch through our own correctly-quoted launcher rather than CMTK's,
    # which breaks on any install path containing a space.
    launcher = staging / CMTK_MACOS_LAUNCHER_RELPATH
    launcher.write_text(CMTK_MACOS_LAUNCHER_SHIM)
